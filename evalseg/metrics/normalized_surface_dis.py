...
class NSD(MetricABS):
    ...
    def set_reference(self, reference: SegmentArray, **kwargs):
        ...
        mask_roi = geometry.one_roi(dense, margin=20, return_index=True)
        gt_dst = np.abs(geometry.distance(dense, spacing=reference.voxelsize, mask_roi=mask_roi, mode="both"))
        self.gt_border_tau = SegmentArray(gt_dst < self.tau)
    ...
    def evaluate(self, test: SegmentArray, **kwargs):
        ...
        pr_border_tau = SegmentArray(pr_dst < self.tau)
        pr_border = SegmentArray(geometry.find_binary_boundary(pred_dense, mode="inner"))
